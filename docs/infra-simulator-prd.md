...
**v0.5에서 교육 비중을 올린다.** PoC 단계에서는 장비 신뢰도 확보를 뒤로 미루고 가시성을 앞세우기로 했으므로, 프리셋과 화면이 인프라 아키텍처를 가르치는 몫이 커진다. 비목표인 "애플리케이션 아키텍처 교육"과는 다르다. 이 도구가 가르치는 것은 물리 장비의 다차원 한계이지 애플리케이션 설계가 아니다. 정확도를 낮추는 방향의 교육은 하지 않는다 — 모르는 한계를 안전으로 바꾸지 않는다는 원칙이 교육 문구보다 우선한다.
...
| P1-15 | 자원 하나씩을 끈 결과를 미리 계산해 보여준다 | 자원마다 단절·부분 전달·흡수 중 하나로 판정하고, 한계 미확인 때문에 판정이 상한이면 표시한다. demand의 출발지·목적지는 단일 장애점 집계에서 제외한다 |
...
- 제조사 마크는 **제품 식별 목적으로만** 쓴다(v0.5 결정). 출처·버전·라이선스를 기록하고, 상표권이 각 소유자에게 있으며 제휴나 승인을 뜻하지 않는다는 고지를 함께 둔다(`vendor/brand-logos/NOTICE.md`). 마크를 장비 식별 이외의 자리(머리말, 소개, 신뢰 배지)에 놓지 않는다.
...
**웹 임베딩.** 설계를 문서나 위키에 살아있는 다이어그램으로 끼워 넣는 방향이 제기됐다. 앱은 외부 의존이 없는 정적 파일이라 구조를 크게 바꾸지 않고 붙일 수 있다. 결정해야 할 것은 세 가지다. 편집 도구를 감춘 읽기 전용 뷰를 둘지, 설계를 URL로 주입할지 파일로만 받을지, iframe 안에서 캔버스 크기를 어떻게 잡을지. 어느 것도 계산 계약을 바꾸지 않는다.
