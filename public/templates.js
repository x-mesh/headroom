...
export const templates = [
  {
    ...
    summary: 'SPINE A/B를 ECMP로 묶었지만 전원은 PDU-3 하나에서 받습니다.',
  ...
  },
  {
    ...
    summary: '총용량은 그대로 두고 절반짜리 장비 두 대로 나눠 받습니다.',
  ...
  },
  {
    ...
    group: 'balance',
    grade: { verdict: 'single-point', severs: 4 },
    summary: '요청도 응답도 로드밸런서를 지나는 풀 프록시입니다.',
  ...
  },
  {
    ...
    group: 'basics',
    grade: { verdict: 'single-point', severs: 4 },
    summary: '웹·앱·데이터 계층을 차례로 지납니다.',
    teaches: '계층마다 먼저 차는 축이 다릅니다. 웹은 세션, 앱은 NIC 패킷, 데이터는 NIC 대역폭에서 한계를 만납니다.',
    tags: ['웹', '계층', '데이터베이스'],
    experiment: {
  ...
  },
  {
    ...
    group: 'basics',
    grade: { verdict: 'single-point', severs: 6 },
    summary: '스파인 2대와 리프 3대를 모두 이은 클로스 팹릭입니다.',
    teaches: 'East-West 트래픽이 두 스파인으로 갈립니다. 한 대가 빠지면 남은 쪽이 그 몫까지 받아야 합니다.',
  ...
  },
  {
    ...
    summary: '방화벽을 지난 트래픽이 WAF를 한 번 더 지납니다.',
    teaches: '체인에서 먼저 차는 것은 대역폭이 아니라 WAF의 TLS 신규 핸드셰이크입니다. 방화벽은 세션 동기화가 없어, 장애가 나면 재수립 폭증까지 함께 계산합니다.',
    tags: ['방화벽', 'WAF', 'TLS', '보안'],
    experiment: {
  ...
  },
  {
    ...
    summary: '원격 근무자가 SSL VPN 게이트웨이를 지나 내부 자원에 닿습니다.',
    teaches: '대역폭과 세션은 절반도 차지 않았는데 동시 VPN 터널이 먼저 한계에 닿습니다. VPN 장비의 사양은 바이트가 아니라 터널 수로 적혀 있습니다.',
    tags: ['VPN', 'SSL', '원격 근무', '터널'],
    experiment: {
      prompt: '접속자가 55% 늘면 무엇이 먼저 한계에 닿을까요?',
      action: { type: 'scale', value: 1.55, label: '배율 1.55배' },
      observe: '대역폭이 아니라 SSL VPN의 동시 세션이 104%가 됩니다. 터널 수가 이 장비의 진짜 정격입니다.',
    },
    build: remoteAccess,
  },
  {
    ...
    summary: '지사를 WAN 회선과 IPsec 게이트웨이로 본사에 잇습니다.',
    teaches: '회선보다 암호화 처리량이 먼저 찹니다. 게이트웨이를 그대로 두고 회선만 늘리면 달라지는 것이 없습니다.',
  ...
  },
  {
    ...
    summary: '외부 방화벽과 내부 방화벽 사이에 DMZ를 둡니다.',
    teaches: '같은 트래픽이 방화벽 두 대를 차례로 지납니다. 둘의 용량이 다르면 판정은 작은 쪽이 합니다.',
    tags: ['방화벽', 'DMZ', 'WAF', '보안'],
    experiment: {
  ...
  },
  {
    ...
    group: 'capacity',
    grade: { verdict: 'single-point', severs: 7 },
    summary: '온프레미스와 클라우드를 WAN 회선으로 잇습니다.',
    teaches: '사이트 안 장비는 넉넉합니다. 그래도 클라우드로 가는 길이 한 가닥이면 거기서 전체가 정해집니다.',
  ...
  },
  {
    ...
    summary: '엣지 캐시가 앞에서 받고, 미스만 오리진으로 넘어갑니다.',
    teaches: '오리진으로 넘어가는 바이트는 얼마 되지 않습니다. 그런데도 먼저 차는 곳은 오리진의 신규 세션입니다. 캐시 적중률이 왜 용량 문제인지가 여기서 보입니다.',
    tags: ['CDN', '캐시', '오리진', '세션'],
    experiment: {
  ...
  },
  {
    ...
    group: 'capacity',
    grade: { verdict: 'single-point', severs: 7 },
    summary: '서비스들이 서로를 호출해 트래픽이 다대다로 흐릅니다.',
    teaches: '호출 하나하나가 작습니다. 대역폭은 남아도는데 스위치가 세는 것은 바이트가 아니라 패킷입니다.',
    tags: ['마이크로서비스', 'East-West', 'PPS', '스위치'],
    experiment: { prompt: '대역폭을 많이 쓰지 않아도 스위치가 포화될 수 있을까요?', action: { type: 'scale', value: 1.25, label: '배율 1.25배' }, observe: '작은 패킷 수가 늘면서 forwarding_pps가 먼저 한계에 닿습니다.' },
    build: microservices,
  },
  {
    ...
    group: 'capacity',
    grade: { verdict: 'single-point', severs: 5 },
    summary: '야간 백업과 아카이브가 스토리지로 몰립니다.',
    teaches: '굵은 플로우 몇 개가 전부입니다. 세션 수로는 한가한데 NIC 대역폭이 먼저 찹니다.',
    tags: ['백업', '스토리지', 'NAS', '대역폭'],
    experiment: {
      prompt: '백업 양이 30% 늘면 무엇이 먼저 찰까요?',
      action: { type: 'scale', value: 1.3, label: '배율 1.30배' },
      observe: '세션은 몇 개 없는데 NAS의 NIC 대역폭이 104%가 됩니다. 백업은 이렇게 굵고 적은 플로우로 옵니다.',
    },
    build: backupNetwork,
  },
  {
    ...
    group: 'balance',
    grade: { verdict: 'single-point', severs: 2 },
    summary: '가상 데스크톱을 브로커 뒤에 둡니다.',
    teaches: '데스크톱은 한 번 붙으면 오래 붙어 있습니다. 새로 열리는 속도보다 동시에 열려 있는 수가 먼저 찹니다.',
  ...
  },
  {
    ...
    group: 'security',
    grade: { verdict: 'single-point', severs: 2 },
    summary: 'WAF 뒤에 결제 애플리케이션과 원장을 둡니다.',
    teaches: '결제는 연결을 잘 재사용하지 않습니다. 매번 새로 맺는 TLS가 대역폭보다 먼저 한계에 닿습니다.',
    tags: ['결제', 'TLS', 'WAF', '보안'],
    experiment: {
  ...
  },
  {
    ...
    group: 'balance',
    grade: { verdict: 'single-point', severs: 2 },
    summary: '오리진에서 엣지를 거쳐 시청자에게 내보냅니다.',
    teaches: '세션 수는 얼마 안 되는데 바이트만 계속 흐릅니다. 대역폭이 먼저 차는, 흔치 않은 설계입니다.',
    tags: ['스트리밍', '대역폭', '엣지', 'CDN'],
    experiment: {
      prompt: '시청자가 25% 늘면 무엇이 먼저 찰까요?',
      action: { type: 'scale', value: 1.25, label: '배율 1.25배' },
      observe: '엣지의 대역폭이 108%가 됩니다. 세션이 아니라 바이트가 먼저 찼습니다.',
    },
    build: streaming,
  },
  {
    ...
    group: 'capacity',
    grade: { verdict: 'single-point', severs: 2 },
    summary: '현장 센서를 게이트웨이로 모아 수집 플랫폼에 넣습니다.',
    teaches: '센서가 보내는 패킷은 하나하나가 아주 작습니다. 대역폭은 9%에 그치는데 게이트웨이의 패킷 처리량이 먼저 찹니다.',
    tags: ['IoT', 'PPS', '게이트웨이', '센서'],
    experiment: {
      prompt: '장치가 15% 늘면 대역폭과 패킷 중 어디가 먼저 찰까요?',
      action: { type: 'scale', value: 1.15, label: '배율 1.15배' },
      observe: '대역폭은 여전히 10% 언저리인데 게이트웨이의 패킷 처리량이 107%가 됩니다. 바이트가 아니라 패킷 수가 한계입니다.',
    },
    build: iotGateway,
  },
  {
    ...
    group: 'capacity',
    grade: { verdict: 'single-point', severs: 7 },
    summary: '주 사이트와 보조 사이트를 좁은 회선으로 잇고 복제합니다.',
    teaches: '복제 트래픽은 사이트 안이 아니라 사이트를 잇는 회선에 얹힙니다. 좁은 쪽이 어디인지 먼저 보세요.',
  ...
  },
  {
    ...
    group: 'evidence',
    grade: { verdict: 'partial' },
    summary: '장비 사용률이 아니라 선언한 서비스 기준으로 판정합니다.',
  ...
  },
  {
    ...
    group: 'evidence',
    grade: { verdict: 'single-point', severs: 8 },
    summary: '카탈로그 데이터시트 값을 그대로 붙이고 우리 트래픽 조건과 대조합니다.',
  ...
  },
  {
    ...
    summary: '경계 2쌍과 4스파인·8리프 팹릭을 한 POD로 묶습니다.',
  ...
  },
  {
    ...
    group: 'balance',
    grade: { verdict: 'single-point', severs: 6 },
    summary: '도구가 대신 정해 주는 것과 내가 적어 줘야 하는 것을 한 화면에 둡니다.',
  ...
  },
  {
    ...
    group: 'scale',
    grade: { verdict: 'partial' },
    summary: '3단 Clos라 동서 경로가 열거 한계를 넘습니다.',
  ...
  },
  {
    ...
    group: 'capacity',
    grade: { verdict: 'single-point', severs: 7 },
    summary: '대역폭이 아니라 랙의 전력 예산이 먼저 찹니다.',
  ...
  },
  {
    ...
    group: 'resilience',
    grade: { verdict: 'single-point', severs: 5 },
    summary: 'A/B 두 레일로 급전하는데 스토리지만 한쪽 레일에 물려 있습니다.',
  ...
  },
  {
    ...
    summary: 'ToR의 100Gbps uplink 두 가닥이 평소에는 부하를 나누고, 하나가 끊기면 남은 하나로 몰립니다.',
  ...
  },
  {
    ...
    group: 'evidence',
    grade: { verdict: 'single-point', severs: 6 },
    summary: '같은 데이터시트 값이라도 조건이 어긋난 축을 수락했는지에 따라 계산에 쓸 수 있는지가 갈립니다.',
    ...
    experiment: {
      ...
      action: { type: 'scale', value: 1.5, label: '배율 1.50배' },
  ...
  },
  {
    ...
    group: 'resilience',
    grade: { verdict: 'single-point', severs: 2 },
    summary: '용량이 같은 HA 방화벽 쌍 둘을 놓고 상태 동기화만 다르게 둡니다.',
