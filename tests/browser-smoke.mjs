...
async function verifyCanvasEditing() {
  ...
  const startAdvanced = page.locator('#start-menu .top-menu-disclosure');
  assert.equal(await startAdvanced.count(), 1);
  assert.equal(await startAdvanced.getAttribute('open'), null, '시작 메뉴 고급 가져오기는 처음에 닫혀 있어야 합니다');
  for (const action of ['new', 'new-blank', 'open']) assert.equal(await page.locator('#start-menu [data-editor-action="' + action + '"]').isVisible(), true);
  for (const action of ['import-drawio', 'fill-capacity']) assert.equal(await page.locator('#start-menu [data-editor-action="' + action + '"]').isVisible(), false);
  await page.locator('#start-menu [data-editor-action="new"]').focus();
  await page.keyboard.press('Tab');
  await page.keyboard.press('Tab');
  await page.keyboard.press('Tab');
  assert.equal(await page.evaluate(() => document.activeElement?.matches('#start-menu .top-menu-disclosure > summary')), true, '고급 그룹 summary는 Tab으로 접근할 수 있어야 합니다');
  await page.keyboard.press('Enter');
  assert.equal(await startAdvanced.getAttribute('open'), '', 'summary 키 입력으로 고급 그룹을 열 수 있어야 합니다');
  for (const action of ['import-drawio', 'fill-capacity']) assert.equal(await page.locator('#start-menu [data-editor-action="' + action + '"]').isVisible(), true);
  await page.locator('#start-menu [data-editor-action="new"]').focus();
  await page.keyboard.press('End');
  assert.equal(await page.evaluate(() => document.activeElement?.dataset.editorAction), 'fill-capacity', '펼친 메뉴의 End는 마지막 고급 항목으로 이동해야 합니다');
  await page.keyboard.press('ArrowDown');
  assert.equal(await page.evaluate(() => document.activeElement?.dataset.editorAction), 'new', '펼친 메뉴의 ArrowDown은 마지막에서 처음으로 순환해야 합니다');
  await startAdvanced.locator('summary').click();
  assert.equal(await startAdvanced.getAttribute('open'), null);
  await page.locator('#start-menu [data-editor-action="new"]').focus();
  await page.keyboard.press('End');
  assert.equal(await page.evaluate(() => document.activeElement?.dataset.editorAction), 'open', '닫힌 메뉴의 End는 보이는 마지막 항목만 선택해야 합니다');
  await page.keyboard.press('ArrowDown');
  assert.equal(await page.evaluate(() => document.activeElement?.dataset.editorAction), 'new', '닫힌 메뉴의 ArrowDown은 숨은 항목을 건너뛰어야 합니다');
  await page.keyboard.press('Escape');
  assert.equal(await page.locator('#start-menu').isHidden(), true);
  await page.locator('#project-menu-button').click();
  const projectAdvanced = page.locator('#project-menu .top-menu-disclosure');
  assert.equal(await projectAdvanced.count(), 1);
  assert.equal(await projectAdvanced.getAttribute('open'), null, '프로젝트 메뉴 고급 가져오기는 처음에 닫혀 있어야 합니다');
  assert.equal(await page.locator('#project-menu [data-editor-action="save"]').isVisible(), true);
  for (const action of ['import-device', 'import-measured-limits', 'import-observed-load', 'import-zabbix-observed-load']) assert.equal(await page.locator('#project-menu [data-editor-action="' + action + '"]').isVisible(), false);
  await projectAdvanced.locator('summary').click();
  for (const action of ['import-device', 'import-measured-limits', 'import-observed-load', 'import-zabbix-observed-load']) assert.equal(await page.locator('#project-menu [data-editor-action="' + action + '"]').isVisible(), true);
  await page.keyboard.press('Escape');
  await page.locator('#export-menu-button').click();
  const exportAdvanced = page.locator('#export-menu .top-menu-disclosure');
  assert.equal(await exportAdvanced.count(), 1);
  assert.equal(await exportAdvanced.getAttribute('open'), null, '내보내기 고급 포맷은 처음에 닫혀 있어야 합니다');
  for (const action of ['result', 'report-markdown']) assert.equal(await page.locator('#export-menu [data-export-action="' + action + '"]').isVisible(), true);
  for (const action of ['export-svg', 'export-png']) assert.equal(await page.locator('#export-menu [data-editor-action="' + action + '"]').isVisible(), true);
  for (const action of ['report-html', 'report-json']) assert.equal(await page.locator('#export-menu [data-export-action="' + action + '"]').isVisible(), false);
  for (const action of ['export-svg-anonymized', 'export-png-anonymized']) assert.equal(await page.locator('#export-menu [data-editor-action="' + action + '"]').isVisible(), false);
  await exportAdvanced.locator('summary').click();
  for (const action of ['report-html', 'report-json']) assert.equal(await page.locator('#export-menu [data-export-action="' + action + '"]').isVisible(), true);
  for (const action of ['export-svg-anonymized', 'export-png-anonymized']) assert.equal(await page.locator('#export-menu [data-editor-action="' + action + '"]').isVisible(), true);
  assert.deepEqual(await page.locator('#export-menu [data-export-action], #export-menu [data-editor-action^="export-"]').evaluateAll((items) => items.map((item) => item.dataset.exportAction || item.dataset.editorAction)), ['result', 'report-markdown', 'export-svg', 'export-png', 'report-html', 'report-json', 'export-svg-anonymized', 'export-png-anonymized']);
  await page.keyboard.press('Escape');
  assert.equal(await page.locator('#export-menu-button').evaluate((button) => document.activeElement === button), true, 'Escape는 내보내기 trigger로 focus를 복원해야 합니다');
  ...
  await page.locator('#start-menu-button').click();
  await page.locator('#start-menu .top-menu-disclosure > summary').click();
...
async function verify(viewport, screenshot, interact = false) {
  ...
  const [svgDownload] = await Promise.all([
    ...
    await page.locator('#export-menu-button').click();
    await page.locator('#export-menu .top-menu-disclosure > summary').click();
